...
/* $Id$ */
	...
	/*
	 * The data for this script generated from one small script. :-)
	 * Based on /etc/mime.types
	 */

	final class MimeTypeMap extends Enumeration
	{
		...
		/** (non-PHPdoc)
		 * @see core/Base/IdentifiableObject::wrap()
		 * @return MimeTypeMap
		 */
		...
		/**
		 * Return MimeTypeMap object by mime-type string
		 * @param string $value
		...
		{
			$any = new self( self::getAnyId() );
			$list = $any->getNameList();
			
			$value = mb_strtolower( $value );
			
			foreach ( $list as $key => $mimeType )
			{
				if( mb_strtolower( $mimeType ) == $value )
					return new self($key);
		...
		/**
		 * Return MimeTypeMap object by extension without [dot] prefix
		 * @param string $value
		 * @throws ObjectNotFoundException
		 * @return MimeTypeMap
		 */
		public static function getByExtension($value)
		{
			...
			{
				if( mb_strtolower( $extension ) == $value )
					return new self($key);
