	...
	**/
	final class ArrayUtils extends StaticFactory
	{
		...
		**/
		...
		/**
		 * in array(2) { 'a' => array(2) { 0 => 1, 1 => 2 }, 'b' => array(2) { 0 => 5, 1 => 7 } }
		 * out array(2) { 0 => array( a => 1, b=>5 ), 1=> array() { a=>2, b=>7 } }
		 *
		 * @param array $array
		 * @return array
		 */
		static public function normilizeArray(array $array) {
			...
			foreach ($keys as $key => $keyName) {
				foreach ( $values[$key] as $vkey => $value ) {
					$return[$vkey][$keyName]=$value;
				}
			}
			unset($keys);
			unset($values);
