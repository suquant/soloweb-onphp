...
/* $Id$ */
	...
	*/
	class UUIDUtils extends StaticFactory
	{
		...
		/*
		 * Quad-word, 128-bit (not impl.)
		 */
		const FMT_QWORD = 1;
		
		/*
		 * Double-word, 64-bit (not impl.)
		 */
		const FMT_DWORD = 2;
		
		/*
		 * Word, 32-bit (not impl.)
		 */
		const FMT_WORD = 4;
		 ...
		 * @param $ns
		 */
		static public function generate($type = self::UUID_TIME, $fmt = self::FMT_STRING, $node = "", $ns = "")
		{
		...
		static private function generateNameMD5($ns, $node) {
			return 
				self::generateName($ns, $node, "md5", self::UUID_NAME_MD5);
		}
